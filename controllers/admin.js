...
exports.postUpdateBook = async (req, res, next) => {
  try {
    ...
    const currentBook = await Book.findById(book_id);

    //Update image start
    if (req.file) {
      imageUrl = `${uid()}__${req.file.originalname}`;
      let filename = `images/${imageUrl}`;
      let previousImagePath = `images/${currentBook.image}`;
...
exports.postAddNewBook = async (req, res, next) => {
  try {
    ...
    if (req.file) {
      imageUrl = `${uid()}__${req.file.originalname}`;
      let filename = `images/${imageUrl}`;
      let previousImagePath = `images/${new_book.image}`;
      ...
      imageUrl = "defaultbook.png";
    }
    // Image upload end

    new_book.image = imageUrl;
    await new_book.save();
    req.flash(
      "success",
