...
// new addition to get top 10 books
exports.getTenBooks = async (req, res, next) => {
  try {
    const books = await Book.find().sort({ createdAt: -1 }).limit(10);
    const booksBorrow = await Book.find().sort({ borrowed: -1 }).limit(10);

    res.render("landing", { books: books, booksBorrow: booksBorrow });
